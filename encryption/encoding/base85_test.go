...
func TestBase85(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		...
		recoveredNonce, recoveredCipherText, err = encoder.Deserialize(ctx, base85Encrypted, authTagSize, nonceSize)
		assert.NoError(t, err)
	...
	t.Run("truncated encrypted text", func(t *testing.T) {
		encoder := encoding.NewBase85()
		_, _, err := encoder.Deserialize(ctx, base85Encrypted[:15], 16, 12)
		assert.ErrorContains(t, err, "truncated")
	})

	t.Run("invalid base85 encoding", func(t *testing.T) {
		encoder := encoding.NewBase85()
		_, _, err := encoder.Deserialize(ctx, []byte("invalid base85!"), 16, 12)
		assert.Error(t, err)
	})
}
